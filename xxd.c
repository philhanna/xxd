...
static int huntype( FILE *fpi, FILE *fpo, int cols, int hextype, long base_off) {
    ...
    while ((c = getc(fpi)) != EOF) {
        ...
        if (n2 >= 0 && n1 >= 0) {
            ...
            want_off++;
            n1 = -1;
            if (!hextype && (++p >= cols)) /* skip the rest of the line as garbage */
                c = skip_to_eol(fpi, c);
        }
        else if (n1 < 0 && n2 < 0 && n3 < 0) /* already stumbled into garbage, skip line, wait and see */
            c = skip_to_eol(fpi, c);
...
int main(int argc, char *argv[]) {
    ...
    while ((length < 0 || n < length) && (e = getc_or_die(fp)) != EOF) {
        ...
        if (p == 0) {
            addrlen = sprintf(l, decimal_offset ? "%08ld:" : "%08lx:", ((unsigned long)(n + seekoff + displayoff)));
